...
- **"📏 Ruler"** for row/column numbers alongside the page, **"🗖 Focus"** to hide the source editor and concentrate on the preview, **"🔍 Fit to Screen"** to scale the whole page down (or up) so content placed far down a tall page is visible without scrolling, and a **Zoom** box (5–200%, in steps of 5) to pick the scale yourself when Fit to Screen leaves things too small.
- **"✏️ Edit Field" / "✏️ Edit Text"** edits the selected item: a field's name, type and size from one small menu (with the size kept consistent with the type — e.g. no decimals on a character field), or a constant's text. The same "Edit Field..." is in the field's tree menu.
- **"⚙ Configuration"**: choose which levels' keywords the preview lists (file / record / field-constant), and the decimal/thousands-separator (US/European) and date-separator (US `/` / European `-`) conventions the preview edits numeric fields with — set by hand, fetched from a connected IBM i's `QDECFMT`/`QDATSEP` system values, or reset to default. Persists across sessions.
...
- **Fields** — Edit Field (name, type and size in one menu), Rename, Copy, Delete, edit Attributes, edit Indicators.
